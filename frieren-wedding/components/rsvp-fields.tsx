...
export const emptyDraft:Draft={name:'',phone:'',guests:'1',needsStay:null,stayGuests:'1',checkIn:wedding.rsvp.stayMin||'',checkOut:wedding.rsvp.stayMax||'',notes:''};
...
export function RsvpFields({value,onChange,disabled=false}:{value:Draft;onChange:(value:Draft)=>void;disabled?:boolean}){
  ...
  </div><div className="stay-choice"><p id="stay-question">是否需要住宿 <span>*</span></p><div role="group" aria-labelledby="stay-question"><Button type="button" variant={value.needsStay===false?'default':'outline'} aria-pressed={value.needsStay===false} onClick={()=>change('needsStay',false)}>无需住宿</Button><Button type="button" variant={value.needsStay===true?'default':'outline'} aria-pressed={value.needsStay===true} onClick={()=>onChange({...value,needsStay:true,checkIn:wedding.rsvp.stayMin||'',checkOut:wedding.rsvp.stayMax||''})}>需要安排住宿</Button></div></div>
    {value.needsStay&&<div className="stay-details"><p className="muted">先登记你的需求，具体住宿安排由新人后续联系确认。</p><div className="form-grid"><label className="full-field">住宿人数 <span>*</span><Input type="number" inputMode="numeric" min={1} max={Number(value.guests)||1} value={value.stayGuests} onChange={e=>change('stayGuests',e.target.value)} required/></label><div className="fixed-stay-date"><span>入住日期</span><strong>2026 年 10 月 24 日</strong></div><div className="fixed-stay-date"><span>离店日期</span><strong>2026 年 10 月 25 日</strong></div></div></div>}
