...
export function checkOrigin(request:Request){const origin=request.headers.get('origin'),expected=process.env.SITE_ORIGIN||new URL(request.url).origin;if(origin!==expected)throw new AppError('请求来源无效，请重新打开邀请函。',403);}
export async function readBody(request:Request):Promise<Record<string,unknown>>{checkOrigin(request);if(!request.headers.get('content-type')?.startsWith('application/json'))throw new AppError('请求格式不正确。',415);if(Number(request.headers.get('content-length'))>8192)throw new AppError('提交内容过长。',413);const text=await request.text();if(new TextEncoder().encode(text).length>8192)throw new AppError('提交内容过长。',413);try{const body=JSON.parse(text);if(!body||typeof body!=='object'||Array.isArray(body))throw 0;return body;}catch{throw new AppError('提交内容格式不正确。');}}
